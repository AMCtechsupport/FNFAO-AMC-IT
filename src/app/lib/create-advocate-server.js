...
export const createAdvocate = async ({
  ...
  try {
    ...
    try {
      ...
      console.log(
        "Checking for existing users with email:",
      ...
      const emailInvitations = existingInvitations.data.filter(
        (inv) =>
          inv.emailAddress &&
          inv.emailAddress.toLowerCase() === email.toLowerCase().trim(),
      );
      console.log(`Invitations for ${email}:`, emailInvitations.length);

      if (emailInvitations.length > 0) {
        console.log(
          `Found ${emailInvitations.length} existing invitations for this email. Canceling them...`,
        );
        for (const invitation of emailInvitations) {
          try {
            ...
            console.log(
              `Canceled invitation ${invitation.id} for ${invitation.emailAddress}`,
            );
          } catch (revokeError) {
            console.log(
              `Failed to cancel invitation ${invitation.id} (might already be expired/used):`,
              revokeError.message,
            );
