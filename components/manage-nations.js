...
const FirstNationManagement = () => {
  ...
  const handleEditFirstNation = async (oldValue) => {
  ...
  const filtered = firstNations.filter((nation) =>
    nation.firstNationMembership.toLowerCase().includes(search.toLowerCase())
  );
                ...
                filtered.map((nation, index) => (
                  ...
                  >
                    <td className="py-3 px-4 font-medium text-gray-800 align-middle">
                      {editingItem === nation.firstNationMembership ? (
                        <input
                          type="text"
                          value={editValue}
                    ...
                    <td className="py-3 px-4 align-middle whitespace-nowrap w-px text-center">
                      {editingItem === nation.firstNationMembership ? (
                        <div className="flex items-center justify-center gap-2">
                          <button
                            onClick={() => handleEditFirstNation(nation.firstNationMembership)}
