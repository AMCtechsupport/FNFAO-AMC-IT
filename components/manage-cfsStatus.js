...
const CFSStatusManagement = () => {
  ...
  const handleEditCfsStatus = async (oldValue) => {
  ...
  const filtered = cfsStatuses.filter((status) =>
    status.cfsStatus.toLowerCase().includes(search.toLowerCase())
  );
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <h3 className="text-lg font-bold text-gray-900">Manage CFS Statuses</h3>
        <span className="inline-flex items-center gap-1.5 text-xs font-medium px-2.5 py-1 rounded-full" style={{ backgroundColor: "rgba(240, 238, 246, 0.8)", color: "rgba(97, 0, 215, 0.8)", border: "1px solid rgba(178, 179, 215, 0.8)" }}>
          ...
          {cfsStatuses.length} {cfsStatuses.length === 1 ? "entry" : "entries"}
        </span>
      </div>
      ...
      <div>
        <button
          onClick={() => { setShowForm(!showForm); setNewCfsStatus(""); }}
          ...
          onMouseEnter={e => { e.currentTarget.style.backgroundColor = "#ffffff"; e.currentTarget.style.borderColor = "rgba(97, 0, 215, 0.8)"; }}
        ...
        >
          {showForm ? "Cancel" : "+ Add New CFS Status"}
        </button>
          ...
          <table className="min-w-full text-sm text-left">
            <thead>
              <tr className="text-white sticky top-0" style={{ backgroundColor: "rgba(97, 0, 215, 0.8)" }}>
                <th className="py-3 px-4 font-semibold text-xs uppercase tracking-wider">CFS Status</th>
                    ...
                    <td className="py-3 px-4 align-middle whitespace-nowrap w-px text-center">
                      {editingItem === status.cfsStatus ? (
                        <div className="flex items-center justify-center gap-2">
                          <button
                            onClick={() => handleEditCfsStatus(status.cfsStatus)}
                        ...
                        <div className="flex items-center justify-center gap-2">
                          <button
                            onClick={() => { setEditingItem(status.cfsStatus); setEditValue(status.cfsStatus); }}
